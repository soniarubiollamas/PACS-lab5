...
int main(int argc, char **argv)
{
    if (argc != 4)
    {
        printf("Usage: %s <image_path> <output_path> <degrees>\n", argv[0]);
        return -1;
    }

    const char *input_image_path = argv[1];
    const char *output_image_path = argv[2];
    const float degrees = atoi(argv[3]);
    ...
    FILE *fileHandler = fopen("kernel_rotate.cl", "r"); // put path to kernel.cl
    ...
    // Create the kernel
    cl_kernel kernel = clCreateKernel(program, "image_rotate", &err);
    ...
    err = clSetKernelArg(kernel, 4, sizeof(int), &degrees);
